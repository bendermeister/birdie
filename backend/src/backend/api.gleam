...
pub fn api(ctx: context.Context, req: wisp.Request) -> wisp.Response {
  case wisp.path_segments(req) {
    ...
    ["api", "song", "upsert"] -> process_song_upsert(ctx, req) |> wrapper_nil()

    // tag endpoints
    ["api", "tag", "get", "all"] -> process_tag_get_all(ctx) |> wrapper_nil()
    ["api", "tag", "delete", id] -> process_tag_delete(ctx, id) |> wrapper_nil()
    ["api", "tag", "upsert"] -> process_tag_upsert(ctx, req) |> wrapper_nil()
    ...
    ["api", "album", "upsert"] ->
      process_album_upsert(ctx, req) |> wrapper_nil()
    ...
    ["api", "artist", "upsert"] ->
      process_artist_upsert(ctx, req) |> wrapper_nil()
...
fn process_song_upsert(ctx: context.Context, req: wisp.Request) {
  req_body_parse(ctx, req, song.json_decoder())
  |> result.try(db.song_upsert(ctx, _))
...
fn process_tag_upsert(ctx: context.Context, req: wisp.Request) {
  req_body_parse(ctx, req, tag.json_decoder())
  |> result.try(db.tag_upsert(ctx, _))
...
fn process_album_upsert(ctx: context.Context, req: wisp.Request) {
  req_body_parse(ctx, req, album.json_decoder())
  |> result.try(db.album_upsert(ctx, _))
...
fn process_artist_upsert(
  ctx: context.Context,
  req: wisp.Request,
) -> Result(wisp.Response, Nil) {
  req_body_parse(ctx, req, artist.json_decoder())
  |> result.try(db.artist_upsert(ctx, _))
